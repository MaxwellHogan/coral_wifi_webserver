...
constexpr char ktestMethodPrefix[] = "/test_method";
...
constexpr float kThreshold = 0.6;
...
HttpServer::Content UriHandler(const char* uri) {
  ...
  else if (StrEndsWith(uri, ktestMethodPrefix)) {

    Blink(1, 25);
    printf("SUCCESS! \r\n");
    std::array<uint8_t, 3> tx_data = {0x69, 0x69, 0xcd};
    // transfer spi 
    CHECK(SpiTransfer(config, tx_data.data(), NULL, 3));

    std::vector<uint8_t> test_message = {0x69};
    return test_message;
...
[[noreturn]] void InferenceTask(void* param) {
  ...
  while (true) {
    ...
    std::vector<tensorflow::Object> bbox_list;
    ...
    if (xSemaphoreTake(img_mutex, portMAX_DELAY) == pdTRUE) {
      ...
      // get results 
      bbox_list = tensorflow::GetDetectionResults(&interpreter, kThreshold, kTopK);
    ...
    std::string bbox_string = "{\"dtime\": " + std::to_string(dtime) + ", ";
      ...
      for (unsigned int i = 0; i < num_bboxes_output; ++i) {
        int class_id = static_cast<int>(bbox_list[i].id);
        bbox_string += "{\"id\": " + std::to_string(class_id) + ", ";
        bbox_string += "\"score\": " + std::to_string(bbox_list[i].score) + ", ";
        bbox_string += "\"xmin\": " + std::to_string(bbox_list[i].bbox.xmin) + ", ";
        bbox_string += "\"ymin\": " + std::to_string(bbox_list[i].bbox.ymin) + ", ";
        bbox_string += "\"xmax\": " + std::to_string(bbox_list[i].bbox.xmax) + ", ";
        bbox_string += "\"ymax\": " + std::to_string(bbox_list[i].bbox.ymax) + "}";
...
void Main() {
  ...
  Blink(3, 500);
  #if DEBUG
    printf("Object detection inference and HTTP server over Wifi\r\n");
  #endif
